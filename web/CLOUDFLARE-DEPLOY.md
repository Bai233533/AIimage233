...
├── workers/                      # 备用：单 Worker 模式（不需要）
├── api/                          # 备用：Vercel Serverless（不需要）
├── wrangler.toml                 # 备用：Workers 配置
├── package.json                  # 备用：Node 依赖
...
> ⚠️ 这一步必须先做！否则 API 调用时会报 "no such table" 错误
...
| 配置项 | 值 |
|--------|-----|
| 项目名称 | `ai-drama-master`（或自定义，决定了默认域名） |
| 生产分支 | `main` |
| 框架预设 | **None** |
| 构建命令 | 留空 |
| 构建输出目录 | **`web`** |
...
> ⚠️ **构建输出目录必须填 `web`**，否则找不到 index.html
...
> ⚠️ **Variable name 必须填 `DB`**，因为 `functions/api/[[route]].js` 用的是 `env.DB`
...
| 变量名 | 值 |
|--------|-----|
| `DOUBAO_API_KEY` | `ark-f93f0cb1-d06a-4bf5-af7b-00787df51ebc-672cd` |
| `SEEDREAM_API_KEY` | `ark-f93f0cb1-d06a-4bf5-af7b-00787df51ebc-672cd` |

3. 点击 **保存**

> 💡 代码里已有 API Key，这里主要是给生产环境覆盖默认值，确保生效
...
| 卡密兑换 | 我的 → 卡密兑换 → 输入 `VIP-TEST-1234` | 提示兑换成功，会员延长 |
| AI 对话 | 创建对话 → 输入"帮我生成一只猫" | 2-3 秒返回结果 |
| 生成分镜 | 创建对话 → 输入"分镜：一只猫在窗台" | 2-3 秒返回分镜 |
...
### Q1: 部署后访问 404
- 检查"构建输出目录"是否填 `web`
- 检查 Pages 项目根目录是否能看到 `index.html`
...
- D1 binding 没设置或变量名不是 `DB`
- 回到"四、绑定 D1 数据库"

### Q4: AI 请求失败 "401 Unauthorized"
- 环境变量没设置或 Key 错误
- 重新设置 `DOUBAO_API_KEY` 和 `SEEDREAM_API_KEY`

### Q5: 部署失败 "Build failed"
- 因为构建输出目录为空导致，Pages 没找到文件
- 确保 git push 时 `web/index.html` 在仓库里
